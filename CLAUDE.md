...
- `apps/api`: the full `auth` module (login, rotating refresh with reuse detection, logout, current session, rate limiting, audit logging — see §12), DB-backed RBAC, the `LECTURA_BALANCE` slice end to end (Repository → Service → Controller → routes), the daily closing job, and user management (§13). All of it is **verified end to end against the real database**; the `USUARIO` table is currently empty, so getting in means running the bootstrap command below.
- `apps/web`: login and the forced password change are built and served; the visual identity is the prototype's control-room palette loaded as shadcn tokens. No operational screens yet — Balance Diario is next.
...
**Not built yet**: Balance Diario, the first operational screen — its backend is ready and it is the next thing to build. Also missing: the remaining Despacho slices (`LECTURA_FUENTE`, `QUEMA_NACIONAL`, `NOVEDAD_OPERATIVA`, `CONTACTO`, and the two reports), the Mantenimiento/Actividades API, and the superadmin's user management screen.
