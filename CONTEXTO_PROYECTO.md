...
9. Seed data real: 9 sistemas, 4 regiones de Despacho, 6 regiones de Mantenimiento, catálogos iniciales de `INSUMO`/`PRODUCTO_SERVICIO` de Mantenimiento (de `ACTIVIDADES_MDC_FINAL_V4.xls`), valores iniciales de `ESTADO_TELEMETRIA` por dimensión, `SECTOR_CLIENTE` (Empresa Mixta, Petrolero, Eléctrico, Siderúrgico, Petroquímico, Cemento, Otros — decisión #36), y `FUENTE` con la granularidad real de ~22 filas confirmada en la decisión #39 (no la versión simplificada asumida antes).
