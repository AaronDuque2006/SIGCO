# Contexto del Proyecto — SICOG (Sistema de Información para la Gerencia de Control Operacional de Gas, PDVSA Gas)
...
33. **Nombre oficial del sistema: SICOG** (~~Sistema de Control Operacional de Gas~~ — **el nombre desarrollado lo corrige la decisión #109**; la sigla sigue) — elegido por no chocar con nombres de sistemas reales ya existentes en la operación (`SISUGAS`, `WEBGAS`, `Infoplus21`) y por no favorecer a un solo dominio.
...
39. **Granularidad real del catálogo `FUENTE` — resuelve el pendiente §9.6 #7, corrige la afirmación "sin gaps" de la sección 5.** Una segunda auditoría más profunda de `NUEVO_BALANCE_ACTUALIZADO.xlsm` (hojas `FUENTES` y `GUIA WEB`) y confirmación explícita del owner del proyecto establecen que `FUENTE` necesita bastante más granularidad de la asumida originalmente — sigue sin ser cambio de schema (`FUENTE` ya es simple: `id/nombre/sistema_id`, decisión #8), es una corrección al alcance real del **seed data**:
    - **Revisada por la decisión #107**: el catálogo salió de la hoja `FUENTES`, que el balance ya no suma. Hoy son 14 fuentes tomadas de `GUIA WEB`; de esta lista quedan sólo los dos trenes de San Joaquín.
...
41. **Asignación `sistema_id` real de las ~22 filas de `FUENTE` (decisión #39)**, resuelta con `Manual DAO.pptx` (nomenclatura de estaciones por sistema, slides 9/24/43/87-89):
    - **Revisada por la decisión #107**: de estas filas quedan San Joaquín Tren A y B y Tren C; el resto salió del catálogo.
...
47. **Las Empresas Mixtas/LIC son `FUENTE` *y* `CLIENTE` a la vez — corrige la decisión #20, que sólo las contemplaba como cliente.** Evidencia: los volúmenes no coinciden entre las dos hojas (Petro Monagas **aporta 60.61** en el bloque "APORTE" de `FUENTES` pero **consume 12** como cliente en `CEN-ORI`; Petropiar aporta 0 y consume 39), o sea son dos flujos distintos de la misma contraparte — normal en gas: la empresa produce gas asociado que entrega al sistema, y además consume gas para su operación. El Manual DAO lo confirma sin ambigüedad: la **slide 27 se titula "Puntos De Recepción De Gas — FUENTES QUE APORTAN GAS AL SISTEMA"** y lista a Petro Piar, PetroMonagas y Petro Delta.
    - **Revisada por la decisión #107**: PAGMI salió del catálogo de fuentes (no está en `GUIA WEB` ni entra en ninguna suma), así que como `FUENTE` quedan 8 con Cardón IV.
...
62. **Composición del Balance Nación, y las tarjetas de resumen de Balance Diario.** Confirmado por el owner el 2026-09-14, a partir de su pedido de ver arriba de la grilla el volumen manejado, el recibido, el entregado y si el sistema está empacado.
    - **`recibido`** = suma de las lecturas de `LECTURA_FUENTE` del día (volumen + desvío) más las entregas directas del corte (decisión #107). Las fuentes no tienen tipo de corte (una lectura por día), así que no se filtran por él.
...
98. **`LECTURA_FUENTE` gana un segundo campo, `procesado` — reabre la decisión #4.** El owner mostró una captura del workbook real: las plantas que procesan gas (San Joaquín Tren A/B, San Joaquín Tren C, Santa Bárbara Tren A/B, Santa Bárbara Tren C, Jusepín, El Tablazo LGN1, El Tablazo LGN2) tienen columnas `PROCESADO` y `RESIDUAL` separadas, no una sola. Confirmado con el owner que **el volumen del corte no cambia de significado**: sigue siendo el dato que entra al balance (recibido, decisiones #62/#74); `procesado` es puramente informativo, se pierde el dato si no se guarda, pero no afecta ningún cálculo. Y que sólo aplica a esas 7 plantas — el resto de las 31 fuentes (entregas directas, empresas mixtas) no procesan nada y no muestran el campo, igual que en el Excel real (donde esa columna queda gris/vacía, no en cero). Nuevo campo `Fuente.procesaGas` (marcado en el seed y en una migración de datos sobre las 7 fuentes existentes por nombre exacto) decide si la pantalla ofrece el segundo cuadro. `LECTURA_FUENTE.procesado` es `Decimal(14,4)?`; su historial gana `procesadoAnt` con el mismo criterio que `horaLecturaAnt`. La decisión #5 (Desvío como `FUENTE` aparte) no se tocó: sigue sin ser una columna.
    - **Revisada por la decisión #107**: sólo San Joaquín Tren A y B y Tren C siguen con `procesaGas` (Santa Bárbara, Jusepín y El Tablazo salieron del catálogo), y esas dos ganan además `desvio`, que sí suma al recibido.
...
107. **El catálogo `FUENTE` es el de la hoja `GUIA WEB`, no el de la hoja `FUENTES` — revisa las decisiones #39, #41, #47 y #98** (confirmado por el owner el 2026-09-24). El owner mostró el bloque "ORIENTE Y OCCIDENTE" del balance y pidió que se analizara `GUIA WEB` porque es de ahí que se alimenta. Leyendo las fórmulas: la tabla es `PROMEDIO!C17:G27`, y cada fila apunta a una celda de la guía; el total recibido es `E27 = F26+G26` (**residual + desvío**, sin el procesado). La hoja `FUENTES`, de donde había salido el catálogo de 31, es un desglose más viejo que el balance ya no suma — y de hecho `EJECUTIVO PUNTUAL` en esta copia todavía lee de ella y da otro recibido (1690,4 contra 1792,35).
    - **Quedan 14 fuentes**: San Joaquín Tren A y B y Tren C (las únicas con `procesaGas`); **Directo a ventas en Tonoro** (`GUIA WEB!G15`, "TRASEGADO"; en `FUENTES` era el Corredor Jusepín-Criogénico) y **Directo a ventas en Anaco** (`F15`, "ATA"; en `FUENTES` era SJB FI FII); **Gas seco (Soto)** (`G3`, "TOTAL SOTO"); las 7 empresas mixtas de "CONVENIOS Y ASOCIACIONES" por separado (Petro Monagas, Mavegas, Bitor, Petropiar, Petro Delta, Gas Guárico, Ypergas); **Cardón IV** y **Producción Occidente** (`F10`, "EyP LA PAZ"), que el balance muestra en una sola fila pero son dos datos. El sistema de las cuatro nuevas sale del `Manual DAO.pptx`, a pedido del owner: **Tonoro → Jusepín - Criogénico** (estaciones Tonoro Viejo/Nuevo, TOV/TON, slide 43); **Anaco → Anaco - Puerto Ordaz** (ATA está en la Estación Principal Anaco del esquema de la slide 23, y "Anaco" es fuente recibida en EPA en la slide 27); **Gas seco (Soto) → Anaco - Puerto Ordaz** ("Norte de Monagas" recibido en la estación Soto, slide 27; en la guía es "SOT-EPA"); **Producción Occidente → Ulé - Amuay** (EyP en el esquema de la slide 79). Anaco y Soto se habían inferido primero de la fuente vieja que reemplazaban y habían quedado mal; los corrige la migración `20260924170000_sistema_fuentes_segun_manual_dao`.
    - **Salen 21**, con sus lecturas (eran las de demostración): RECAT SJ y SJB FI FII (en la guía son *consumo de gas combustible*, no aporte); Santa Bárbara Tren A y B, Tren C y Jusepín (la guía tiene su bloque, vacío, y el balance no los suma — el owner pidió no ponerlos); Soto, Aguasay 5A, Bajo Guanipa, ETSJ, Zapato Viejo y Corredor Jusepín-Criogénico; El Tablazo LGN1/LGN2 y sus 7 salidas; y PAGMI, que no aparece en la guía. El Corredor además era un **subtotal** de otras cinco (`FUENTES!L25`): con el catálogo viejo, cualquier dato cargado ahí contaba doble en el recibido.
    - **`LECTURA_FUENTE` gana `desvio`** (`Decimal(14,4)?`, con `desvioAnt` en el historial), sólo en las fuentes con `procesaGas`. A diferencia del procesado **sí entra en el recibido**: el owner lo confirmó ("aporta al volumen manejado por la fuente, que sí suma al balance"). Un desvío mal escrito frena el guardado igual que el volumen; el procesado no, porque es informativo.
    - **Entregas directas no es una fuente**: `PROMEDIO!F25 = CEN-ORI!E56+E115+E51+E62`, cuatro **clientes** — Cementos Cerro Azul, P.E. San Diego de Cabrutica, La Toscana San Vicente y P.E. Termo Barranca. El workbook los resta del total de su región y los vuelve a sumar al transportado como "ENTREGAS DIRECTAS ORI." (`PROMEDIO!C40`), así que cuentan **una vez de cada lado**. SICOG ya los tenía en el transportado; ahora `CLIENTE.entregaDirecta` los marca y el recibido les suma sus lecturas del mismo corte. El owner aceptó calcularlo así en vez de digitarlo aparte, con la condición de que se vea en el reporte: `BalanceNacionDto.entregasDirectasMmpced`, que la tarjeta de Recibido de Reportes muestra como "incluye N de entregas directas".
    - **Recibido** = Σ (`volumenMmpced` + `desvio`) de `LECTURA_FUENTE` del día + Σ `LECTURA_BALANCE` de los clientes con `entregaDirecta` en el corte pedido. Aplica igual al Balance Nación y a la serie de recibido vs transportado. Migración `20260924160000_fuentes_segun_guia_web`; el seed inserta las cuatro fuentes nuevas.
    - **Fuera de alcance, a propósito**: la guía también trae GPM de entrada/salida y producción de líquidos por planta, y el transportado de `PROMEDIO` lee otras celdas de ella (SOT-EPA, ICO, Ballena…). No se tocaron: el pedido fue el catálogo de fuentes.
108. **RAG: cada usuario ve sus consultas de los últimos 30 días; en la base no se borra ninguna** (confirmado por el owner el 2026-09-24, a propuesta de Claude). "Mis consultas" en `/asistente` muestra sólo las propias, agrupadas por día, con la respuesta y las fuentes **tal como se mostraron** (`consultas_rag.fuentes`, una copia: los `chunk_ids` no alcanzan porque reprocesar un documento regenera sus chunks con otros ids). **30 días** porque las respuestas dependen de los documentos cargados en ese momento, y una vieja puede traer una cifra que ya no vale; cada una ofrece "Preguntar de nuevo". **En la base se conserva todo**: es auditoría, igual que `logs_login` y `logs_intento_no_autorizado`, que tampoco tienen plazo; ocupa poco (~36.000 filas/año a 100 consultas por día) y es de donde salen la bandeja de "no sirvieron" y las preguntas para el §16.9. La retención de todos los registros de auditoría se decide junta, antes del despliegue (§16.10).
109. **Nombre completo del sistema: "Sistema de Información para la Gerencia de Control Operacional de Gas"** (confirmado por el owner el 2026-09-25; corrige el desarrollo de la sigla de la decisión #33, que decía "Sistema de Control Operacional de Gas"). La sigla **SICOG** no cambia y se lee así: **S**istema de **I**nformación + **C**ontrol **O**peracional de **G**as. En el sello del logo el nombre completo no entra legible: arriba va "SICOG" y en el arco de abajo "Gerencia de Control Operacional de Gas", que es lo que la sigla no dice (elegido por el owner entre tres opciones). El nombre completo va en los textos accesibles del sello y del login, en la descripción de la aplicación y en el pie del PDF de reportes.
110. **Logo del sistema** (definido por el owner el 2026-09-25, en varias vueltas sobre el sello del área que trajo — "Despacho Central de Gas"). `apps/web/src/components/logo-sicog.tsx`, dos versiones: **`SelloSicog`** (login, 208 px) — aros, mapa de Venezuela **con la Guayana Esequiba**, la llama sobre el nodo central de una red y los instrumentos de presión, temperatura y válvula; "SICOG" arriba y "Gerencia de Control Operacional de Gas" abajo (decisión #109) — y **`MarcaSicog`** (encabezado, 20 px; la misma figura es `app/icon.svg`, que reemplazó el favicon de Next) — sólo el aro y la llama sobre el nodo. **Con la paleta de la aplicación, no con los colores del sello original** (era naranja y azul): todo sale de los tokens del tema y cambia solo con él. **Animación sólo en el sello del login**: pulsos de luz que salen del nodo central por cada línea de la red y siguen por el tramo exterior, el nodo que late y la llama que parpadea desde la base; todo se apaga con `prefers-reduced-motion`. La marca del encabezado no se anima porque vive en la pantalla de trabajo toda la guardia. En el login el título no se repite a la vista (el sello ya dice el nombre); queda para lectores de pantalla. El mapa es una silueta estilizada, no cartografía exacta; su tamaño se calcula para que ningún punto pase de r=159 del centro del sello.

111. **El hub lleva un globo con la red de gas en la esquina inferior derecha** (pedido del owner el 2026-09-25). `apps/web/src/components/globo-red.tsx`: Venezuela sobre una esfera inclinada, con proyección ortográfica calculada en el propio componente (sin dependencias), el contorno del sello de la decisión #110 (incluida la Guayana Esequiba) pasado a longitud/latitud, y como nodos las plantas y terminales que nombran los siete sistemas del seed. Por cada ruta sale un pulso de luz desde Anaco, el mismo efecto de flujo del sello; Anaco late. El mapa se dibuja 1,6 veces más grande que a escala sobre el mismo globo (a escala quedaba chico). El globo está quieto: se probó hacerlo girar despacio (todo junto y después sólo los meridianos) y el owner prefirió dejarlo fijo. Lo único que se mueve son los pulsos. Es una ilustración y no cartografía: la red une nodos y no sigue el trazado real de los gasoductos. **Extiende la decisión #85**: el hub tampoco es una pantalla operativa (no tiene cifras ni digitación, es la entrada a los departamentos), así que ahí se permite el movimiento. La regla para las pantallas de trabajo sigue igual: ahí no va. Va detrás del contenido, sólo en pantallas `lg` o más anchas, se funde con el fondo por la izquierda y por arriba, toma los colores del tema y se apaga con `prefers-reduced-motion`.
112. **Cada departamento tiene una sección "Acerca de SICOG"** (pedido del owner el 2026-09-25): última entrada del menú lateral de Despacho y de Mantenimiento (`/despacho/acerca`, `/mantenimiento/acerca`), las dos con **la misma página** (`components/acerca-de-sicog.tsx`): el sello, el nombre completo (#109), para qué sirve, **versión y año** (hoy 0.1.0 · 2026, a mano, alineada con los `package.json`) la mención de que se elaboró durante las pasantías del autor en la Gerencia de Control Operacional de PDVSA Gas, y **créditos y contacto del autor: nombre, correo y teléfono** (sin cargo), con los valores que dio el owner. El contacto lo ve cualquier usuario autenticado. En el menú la entrada va sin subtítulo, a pedido del owner.
113. **El logo de PDVSA Gas entra en cuatro lugares** (pedido del owner el 2026-09-25: "en partes estratégicas, sin romper la armonía visual"). `components/logo-pdvsa.tsx`, pasado a SVG desde el EPS que trajo el owner. **En la aplicación va en la tinta del tema, la misma de los botones, sin el rojo oficial**: en el encabezado de las pantallas autenticadas (separado de los controles por un filete, oculto en pantallas angostas), en el pie de la tarjeta del login y en el bloque de créditos de "Acerca de SICOG" (#112). Se probó el rojo en el login y en "Acerca de", y la tinta tenue en el encabezado; el owner cambió las tres cosas a la tinta de los botones. La tenue se leía como deshabilitado, y el azul no se usa porque en esta interfaz significa "se puede tocar". **El rojo oficial queda sólo en la cabecera de la vista de impresión del PDF de Reportes** (#96, cuyo membrete completo sigue pendiente), que se imprime en blanco.
114. **La palabra "SICOG" del encabezado va en Rubik 800** (pedido del owner el 2026-09-25, para que pegue con el "PDVSA" del logo, #113; la eligió entre diez fuentes gruesas después de descartar Archivo negra y ancha). Se carga con `next/font` como `--font-marca` y se usa **sólo ahí**: es el logotipo del sistema escrito con letras, no un rol tipográfico nuevo. El resto de la aplicación sigue en Inter.
115. **La bitácora de fallas de Mantenimiento exporta a Excel, una hoja por región** (pedido del owner el 2026-09-25; él eligió las tres opciones). Botón "Exportar a Excel por región" en `/mantenimiento/telemetria/fallas`. Entra **lo que deja el filtro de la pantalla, todas las páginas** (se piden al tocar el botón, de a 100, con `todasLasPaginas`), no sólo las 50 visibles. El libro trae una hoja **Resumen** (por región: abiertas, resueltas, total y días promedio de caída de las abiertas, más el total nacional) y **una hoja por cada una de las seis regiones de Mantenimiento, aunque quede vacía** — una región sin hoja se leería como que no se exportó. Cada hoja arranca con el filtro aplicado en palabras; dentro de cada región van primero las abiertas y las que más días llevan. Los días de caída van como número, no como "3 años", para poder sumarlos y ordenarlos. La bitácora y el inventario de **Estaciones** suman además un **filtro de Región** para consultar en pantalla (pedido del owner el mismo día), que acota las áreas del filtro de al lado; las regiones salen del catálogo de áreas (`regionesDeAreas`). **El Excel no cambia con él**: sigue saliendo con las seis hojas, como con cualquier otro filtro. El botón es el mismo "Exportar Excel" de las vistas de Despacho, en su propia fila a la derecha. Mismo mecanismo que la decisión #99 (`xlsx` en el navegador), que ahora admite libros de varias hojas (`exportarLibroExcel`).
116. **El alta de una estación carga su inventario de instrumentos, y la edición lo corrige** (el owner encontró el 2026-09-25 que crear una estación no ofrecía los instrumentos). Como la fila del `INVENTARIO ESTACIONES.xls`: una cantidad por cada uno de los 16 tipos, **en el orden de sus columnas** (Presión, Temperatura, PDT, Válvula principal… Separador — el orden de los ids del seed), enteros de 0 a 999, un vacío es cero y los ceros no se guardan. `POST /estaciones` acepta `instrumentos` y los escribe **en la misma escritura anidada** que la estación, para que nunca quede creada sin inventario porque falló un segundo pedido; un tipo inexistente rechaza el alta entera. La edición del detalle suma el mismo bloque y usa el `PUT /estaciones/:id/instrumentos` que ya existía, sólo si el inventario cambió. Verificado contra la base con una estación de prueba, borrada después.
...
| POST | `/estaciones` | Supervisor+. Trae también `instrumentos` (`[{ tipoInstrumentoId, cantidad }]`, opcional, vacío por defecto) y se guardan en la misma escritura que la estación (decisión #116). |
