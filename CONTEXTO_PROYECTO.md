...
- **La gráfica de "SISTEMAS" del workbook mezcla tres niveles.** Confirmado con el owner el 2026-09-16 y verificado contra la base y el Manual DAO. Sus 9 categorías (`EJECUTIVO PUNTUAL!A36:A44`) son: **4 sistemas** del catálogo, **3 sub-sistemas** (`NOR ORIENTE /SINORGAS`, `COSTA OESTE`, `COSTA ESTE`) y **2 flujos de gasoducto** que no son consumo de clientes (`ENTREGAS DIRECTAS ORI.` e `ICO (MORÓN)`).
    - **Son sub-sistemas, no sub-regiones** (corregido por el owner el 2026-09-16). Cada uno cuelga de **un solo sistema**: NOR ORIENTE/SINORGAS → Anaco-José-PLC-Sinorgas; COSTA OESTE y COSTA ESTE → los dos de Ulé-Amuay. Verificado contra la base: todos los clientes de cada bloque comparten sistema. Que dos sub-sistemas cuelguen del mismo sistema es justamente por qué agrupar por `SISTEMA` no puede producir esa apertura.
    - **La regla de agrupación, confirmada por el owner**: por sub-sistema cuando existe, por sistema cuando no.
    - **El Manual DAO no trae un catálogo único de sub-sistemas.** Se buscó en las 80+ diapositivas. La diapositiva 57 enumera **14 sub-sistemas de Anaco-Caracas-Barquisimeto-Río Seco** (`EPA ALTAGRACIA`, `ALTAGRACIA ARICHUNA`, `GUACARA MORÓN`, `MORÓN BARQUISIMETO`…), que son **tramos entre dos estaciones**. La diapositiva 82, de Ulé-Amuay, agrupa sus puntos de entrega por estación/kilómetro y no usa el término: ahí `Costa Oeste` y `Costa Este` aparecen como puntos bajo `KM-4+600` y `KM-00`.
    - **Los dos niveles no son el mismo.** Los 14 del manual son tramos de tubería; los 3 del workbook son agrupaciones de clientes. Sembrar los 14 ahora crearía un catálogo al que nadie apunta: el workbook mete los clientes de Anaco-CCS-BQTO en los bloques CENTRO-CARACAS y CENTRO/OCCIDENTE, sin abrirlos por tramo. **Falta confirmar si los 3 del workbook son todos los que la operación usa.**
    - **`CEN-ORI` está armada en 9 bloques** con encabezado y total: ANACO-JOSE-ORIENTE, NOR ORIENTE/SINORGAS, PUERTO ORDAZ, CENTRO-CARACAS, CENTRO/OCCIDENTE, COSTA OESTE, COSTA ESTE, ULÉ-AMUAY y ENTREGAS DIRECTAS OCC. El gráfico suma esos totales (algunos bloques tienen sub-totales anidados).
    - **`ICO` es una transferencia entre sistemas, un tren de regulación** (owner). `FUENTES!Q31` es un valor **con signo**: `E31 = IF(Q31>0,0,Q31)*-1` es ICO→NURGAS y `F31 = IF(Q31<0,0,Q31)` es NURGAS→ICO — el signo codifica la dirección. Es el mismo pendiente que la decisión #46 dejó abierto al excluir `TRANSFERENCIA ICO-NURGAS` del catálogo `CLIENTE`; en el workbook esa fila vive dentro del bloque ULÉ-AMUAY como si fuera un cliente.
