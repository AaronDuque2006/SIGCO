...
6. ~~Seed de los clientes reales~~ **Hecho**: 110 clientes (decisión #46). **Quedan 2 dudas abiertas**: (a) la fila `OTROS OCCIDENTE` del grupo "ENTREGAS DIRECTAS OCC" no se cargó porque ese grupo no existe en el Manual DAO y no sé a qué sistema asignarla; (b) las Empresas Mixtas (Petro Monagas, Petropiar, Bitor…) quedaron con sector **Petrolero** según las fórmulas del Excel, pero la decisión #20 dice que su `tipo_sector` es **Empresa Mixta** — hay que resolver si son el mismo cliente o dos flujos distintos. Hoy el sector "Empresa Mixta" tiene 0 clientes.
