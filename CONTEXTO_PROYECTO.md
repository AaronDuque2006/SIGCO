...
- **La gráfica de "SISTEMAS" del workbook mezcla tres niveles.** Confirmado con el owner el 2026-09-16 y verificado contra la base y el Manual DAO. Sus 9 categorías (`EJECUTIVO PUNTUAL!A36:A44`) son: **4 sistemas** del catálogo, **3 sub-regiones** (`NOR ORIENTE /SINORGAS`, `COSTA OESTE`, `COSTA ESTE`) y **2 flujos de gasoducto** que no son consumo de clientes (`ENTREGAS DIRECTAS ORI.` e `ICO (MORÓN)`).
    - **Las 3 sub-regiones son homogéneas**: todos sus clientes comparten región *y* sistema. NOR ORIENTE/SINORGAS → Oriente + Anaco-José-PLC-Sinorgas; COSTA OESTE y COSTA ESTE → los dos Occidente + Ulé-Amuay. Por eso agrupar por región no puede producir esa apertura: Costa Oeste y Costa Este colapsan en "Occidente".
    - **La regla de agrupación, confirmada por el owner**: por sub-región cuando existe, por sistema cuando no.
    - **El Manual DAO no trae una lista de sub-regiones.** Se buscó en las 80+ diapositivas: lo que sí enumera son **14 sub-sistemas**, todos dentro de Anaco-Caracas-Barquisimeto-Río Seco (diapositiva 57), y son tramos de gasoducto entre dos estaciones —`GUACARA MORÓN`, `MORÓN BARQUISIMETO`, `ALTAGRACIA ARICHUNA`…—. Es un nivel **más fino** que el del gráfico y una cosa distinta de las sub-regiones. **Falta confirmar si las 3 del workbook son todas.**
